...
npm run ingest [-- --source main|aicc|cse[,...]] [--pages N] [--limit N] [--no-ai] [--upgrade-prompt]   # real pipeline over all boards (or --source): list -> new/updated/duplicate detection -> DB -> Gemini only where needed; --limit overrides each board's default; --upgrade-prompt also re-runs analyses from an older PROMPT_VERSION
...
- `src/sources/*.ts`: one module per notice board, each returning a `RawNotice` (`src/types.ts`). All boards (main `inhaMainNotice`, college `inhaAiccNotice`, department `inhaCseNotice`) are K2Web sites, so each is a `makeBoardSource()` config over the shared `k2web.ts` (User-Agent, fetch, list and article parsers). Register new boards in `src/sources/index.ts` (the `--source` alias and default limit) and `src/sourceMeta.ts` (kind, college and major; dependency-free, used by the web and by `match.ts`). Sources and selectors are documented in `docs/notice-sources.md`.
...
- `src/ingest.ts`: the ingestion loop (dependencies injected, so it can be tested). `ingestAll()` runs the boards in order. A board's listing failure is caught, so the other boards still run, and an AI stop carries over to later boards. Per notice: fetch → `upsertNotice` (new/updated/unchanged by `content_hash` + title) → AI only if `hasCurrentAnalysis` is false. One failure never stops the run. Quota, bad key or model, or 3 overloads in a row stop AI for that run; the remaining notices are deferred (`pending`) to the next run.
