...
function allowedYears(text: string, entranceYear: number, today: string): { years: Set<number> | null; label: string } {
  ...
  if (years.size === 0) return { years: null, label: '' };
  return { years, label: [...new Set(labels)].join('·') };
}
...
export function matchNotice(profile: Profile, notice: NoticeListItem, today: string): MatchResult {
  ...
  const exclude = (why: string) => (excluded ??= why);
  ...
  if (inTarget.majors.size > 0) {
    if (inTarget.majors.has(profile.major)) {
      score += SCORE.major;
      reasons.push(`${profile.major} 대상`);
      specific = true;
    } else if (inTarget.colleges.has(profile.college)) {
      score += SCORE.college;
      reasons.push(`${profile.college} 대상`);
      specific = true;
    } else {
      exclude(`다른 학과 대상 (${[...inTarget.majors].slice(0, 2).join(', ')})`);
    }
    ...
    if (inTarget.colleges.has(profile.college)) {
      ...
      exclude(`다른 단과대학 대상 (${[...inTarget.colleges].slice(0, 2).join(', ')})`);
    }
  }
  ...
  if (!specific) {
    ...
    if (inTitle.majors.has(profile.major)) {
      ...
      reasons.push(`${profile.major} 공지`);
    } else if (inTitle.colleges.has(profile.college) || inTitle.majorColleges.has(profile.college)) {
      score += SCORE.titleCollege;
      reasons.push(`${profile.college} 공지`);
    }
  }

  // 2. graduate-only
  if (GRAD_ONLY.test(target) && !UNDERGRAD.test(target)) exclude('대학원생 대상');

  // 3. academic year / cohort
  const ex = excludedYears(target);
  if (ex.has(profile.year)) exclude(`${profile.year}학년 제외`);
  const { years, label } = allowedYears(target, profile.entranceYear, today);
  if (years) {
    if (years.has(profile.year) || years.has(-1)) {
      score += SCORE.year;
      reasons.push(`${label} 대상`);
      specific = true;
    } else {
      exclude(`${label} 대상`);
    }
  ...
  for (const m of target.matchAll(/(\d{1,2})\s*학기\s*이상/g)) {
    ...
    if (completedSemesters(profile.year, today) < need) exclude(`${need}학기 이상 이수 조건`);
  }
  ...
  if (excluded) {
    return { relevanceScore: 0, matchLevel: 'none', matchReasons: [excluded], excludedBecause: excluded, basedOnTitleOnly: !a };
  }
  score = Math.min(score, 100);
  const matchLevel: MatchLevel = score >= LEVEL_MIN.high ? 'high' : score >= LEVEL_MIN.medium ? 'medium' : 'low';
  return { relevanceScore: score, matchLevel, matchReasons: reasons, excludedBecause: null, basedOnTitleOnly: !a };
}
